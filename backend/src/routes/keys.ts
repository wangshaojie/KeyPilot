...
const TEST_PROMPT =
  'Say "KeyPilot speed test successful" in exactly those words.';
...
async function testKey(key: APIKey): Promise<SpeedTestResult> {
  ...
  try {
    ...
    if (
      ...
      key.provider === "zhipu" ||
      key.provider === "volcengine" ||
      key.provider === "qwen" ||
      key.provider === "moonshot" ||
      key.provider === "stepfun" ||
...
function getDefaultBaseUrl(provider: string): string {
  switch (provider) {
    ...
    case "azure":
      return ""; // Azure requires custom endpoint configured in key
    case "custom":
      return ""; // Custom provider uses stored baseUrl
    default:
      return "";
  }
}
